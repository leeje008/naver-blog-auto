...
네이버 C-Rank/D.I.A.+ 알고리즘 기준에 맞춘 6개 항목 검증.
"""
...
def validate_seo(draft: dict, target_keyword: str, image_count: int = 0) -> dict:
    ...
    # 가중 평균 계산
    total_score = sum(
        checks[key]["score"] * WEIGHTS[key] for key in WEIGHTS
    )
...
def _check_heading_structure(html: str, keyword: str) -> dict:
    ...
    # H1 검증 (네이버 블로그에서는 제목이 H1이므로 본문에 H1 없는 게 정상)
    if len(h1_tags) == 0:
        score += 20  # 본문에 H1 없음 = 정상
    elif len(h1_tags) == 1:
        score += 15
...
def _check_images(html: str, image_count: int) -> dict:
    ...
    if img_tags:
        ...
        # 플레이스홀더만 있는 경우 (아직 이미지 삽입 전)
        score += 20

    # 이미지 배치 간격 (플레이스홀더 기준)
    if placeholder_count >= 2:
        parts = re.split(r"\[IMAGE_\d+\]", html)
        text_parts = [_strip_html(p) for p in parts[1:-1]]  # 이미지 사이 텍스트
        if text_parts:
